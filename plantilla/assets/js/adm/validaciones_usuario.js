   ...
   /*  var error_ffecha = false;
	...
	//limpiar formulario
		...
		//Insertar nuevo
			...
			$('#myForm').attr('action', '../adm/Empleado_c/insertar');
		});
				...
				$.ajax({
					...
					success: function(response){
						if(response.success){
							...
							$('.alert-success').html('Comunity '+type+' exitosamente').fadeIn().delay(4000).fadeOut('slow');
							showAllEmployee();
						}else{
							alert('Error');
						}
		...
		$('#showdata').on('click', '.item-edit', function(){
			...
			$('#myForm').attr('action', '../adm/Empleado_c/actualizar');
			
			$.ajax({
				type: 'ajax',
				method: 'get',
				url: '../adm/Empleado_c/editar',
				data: {id: id},
				async: false,
				dataType: 'json',
