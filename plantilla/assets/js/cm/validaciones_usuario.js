$(document).ready(function(){
	...
	$("#nombre").focusout(function(){check_nombre();});	
     ...
     var pattern = /^[a-zA-Z ñ.áéíóúäëïöü\'-]+$/;
        ...
        if((pattern.test(correo) && correo.length>0)){
           ...
           $('#nombreError').html("Correcto").css({"font-size": "12px", "color": "#34F458"});
           $("#nombreError").show();
        ...
        $("#nombreError").html("Complete el campo").css({"font-size": "14px", "color": "red"});
        $("#nombreError").show();
	...
	$("#user").focusout(function(){check_usuario();});	
	 ...
	 $('#myForm').attr('action', '../cm/Empleado_c/insertar');
 });
		//guardar
				...
				$.ajax({
					...
					success: function(response){
						if(response.success){
							...
							$('.alert-success').html('Empleado '+type+' exitosamente').fadeIn().delay(4000).fadeOut('slow');
							showAllEmployee();
						}else{
							alert('Error');
						}
		...
		$('#showdata').on('click', '.item-edit', function(){
			...
			$('#myForm').attr('action', '../cm/Empleado_c/actualizar');
			
			$.ajax({
				type: 'ajax',
				method: 'get',
				url: '../cm/Empleado_c/editar',
				data: {id: id},
				async: false,
				dataType: 'json',
