...
function selectempleados(){
    ...
    $.ajax({
        type: 'ajax',
        method: 'get',
        url: '../cm/Asignar_tareas_c/mostrarcm',
        ...
        success: function(data){
            ...
            html2 += " <option value='0' disabled='' selected=''>Selecciona Comunity Manager</option>";
            for(i=0; i<data.length; i++){
            ...
            $('#empleado').html(html2);
                ...
                alert('No hay comunity manager para este nodo');
...
function showAllEmployee(){
    ...
    $.ajax({
        type: 'ajax',
        method:'get',
        url: '../cm/Asignar_tareas_c/mostrartareasasignadas',
        data:{idcampana:idcampana,idNodo:idNodo,id_gc:id_gc},
        async: false,
        dataType: 'json',
        success: function(data){
            if(data){
...
$('#showdata').on('click', '.item-edit', function(){
    ...
    var id_preautorizacion = $(this).attr('data');
    $('#myModal').modal('show');
    ...
    $.ajax({
        type: 'ajax',
        method: 'get',
        url: '../cm/Aprobar_c/form_preaut',
        data: {id_preautorizacion: id_preautorizacion},
        async: false,
        dataType: 'json',
        success: function(data){
            var getUrl = window.location;
            var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];
            console.log(data);
            $('#id_preautorizacion').val(data.id_preautorizacion);
