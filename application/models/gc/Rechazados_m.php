...
class Rechazados_m extends CI_Model {
...
function form_rechazadas(){
	$idpre = $this->input->get('idpreautorizacion');
	$idNodo = $this->input->get('idnodo');
	$cmlogeado= $this->session->userdata('id');
...
function mostrarpubrechazadas(){
	...
	$idCampana = $this->input->get('idcampana');
	$idNodo = $this->input->get('idnodo');
	$query=$this->db->query("SELECT * FROM preautorizacion pd 
	INNER JOIN empleado e on pd.id_cm=e.id 
	WHERE pd.id_campana ='$idCampana' AND pd.id_nodo='$idNodo' 
	AND pd.estatus=1 
	");	
...
function preautorizacion_guardar($info,$id_preautorizacion){
