...
class Asignar_tareas_dc_m extends CI_Model {
	...
	function mostrartareasasignadas(){
        ...
        $query=$this->db->query("SELECT * FROM tareas t INNER JOIN empleado e ON t.id_gc=e.id WHERE t.id_campana ='$idCampana' AND t.id_nodo='$idNodo' AND t.id_dc = '$id_dc'
	...
	function mostrar_tareas_enviadas_a_cm(){
		...
		WHERE t.id_campana ='$idCampana' AND t.id_nodo='$idNodo' AND t.id_gc = '$id_gc' AND statuscm=0 AND statusdc=0");
		if($query->num_rows() > 0){
			return $query->result_array();
		}else{
			return false;
	...
	function mostrar_tareas_enviadas_a_dc(){
		...
		WHERE t.id_campana ='$idCampana' AND t.id_nodo='$idNodo' AND t.id_gc = '$id_gc' AND statuscm=1 AND statusdc=1");
		if($query->num_rows() > 0){
			return $query->result_array();
		}else{
			return false;
	...
	function enviar_tarea_a_disenador_dc($info,$id_tarea){
	...
	function mostrar_tareas_enviadas_a_gc(){
		...
		WHERE t.id_campana ='$idCampana' AND t.id_nodo='$idNodo' AND t.id_dc = '$id_dc' 
	...
	function mostrartareasrechazadas(){
        ...
        $query=$this->db->query("SELECT * FROM tareas t INNER JOIN empleado e ON t.id_gc=e.id 
		WHERE t.id_campana ='$idCampana' AND t.id_nodo='$idNodo' AND t.id_dc = '$id_dc' 
		AND statuscm=0 AND statusgc=0  AND statusdc=1 AND statusrech=1 AND statusrev=0");
		if($query->num_rows() > 0){
			return $query->result_array();
		}else{
			return false;
	...
	function enviar_tarea_revision_dc($info,$id_tarea){
		$this->db->where('id_tarea', $id_tarea);
			$this->db->update('tareas', $info);
			if($this->db->affected_rows() > 0){
				return true;
